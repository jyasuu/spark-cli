...
impl std::str::FromStr for Backend {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            other => bail!("unknown backend '{}'; choose: livy, kubernetes, yarn, standalone", other),
        }
    }
}
...
impl Config {
    ...
    pub fn load() -> Result<Self> {
        ...
        if !path.exists() {
            return Ok(Config { config_path: path, ..Default::default() });
        }
    ...
    pub fn add_profile(&mut self, name: String, profile: Profile) -> Result<()> {
        if self.profiles.contains_key(&name) {
            bail!("profile '{}' already exists; use 'profile remove' then 'profile add' to replace", name);
        }
